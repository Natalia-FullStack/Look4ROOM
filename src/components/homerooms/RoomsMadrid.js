...
export default function RoomsMadrid(){
    ...
    useEffect(()=>{
        const getHabitaciones=async()=>{
          setLoading(true)
         const datos=await getDocs(query(collection(db, 'habitaciones'),where ('comunidadAuto', '==', 'Comunidad de Madrid' )));
         const habitaciones = [];
      ...
      const agregarAFavoritos = async (habitacion) => {
        try {
          ...
          if (!querySnapshot.empty) {
            toast.info("Error al añadir a favoritos. Es posible que ya esté en tu lista de favoritos");
            return;
          } 
          ...
          toast.success("¡Has añadido esta habitación a tu lista de favoritos!");
            ...
            toast.error("Error al añadir a favoritos. Es posible que ya esté en tu lista de favoritos");
